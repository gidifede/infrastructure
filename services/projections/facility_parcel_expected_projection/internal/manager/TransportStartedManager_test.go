...
func TestTransportStartedManager_ManageEvent_OK_FacilityExpectedParcelUpdated(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got, expFacilityExpectedParcel) {
				t.Errorf("TransportStartedManager.ManageEvent(). Read %v, want %v", got, expFacilityExpectedParcel)
			}
		})
	}
}

func TestTransportStartedManager_ManageEvent_OK_FacilityExpectedParcelDoesntExist(t *testing.T) {
	facilityID := uuid.New().String()
	vehicleID := uuid.New().String()
	transportID := uuid.New().String()
	...
	expFacilityExpectedParcel := facilityExpectedParcel1
	...
	tests := []struct {
		...
		{name: "Event OK. Facility Expected Parcel Doesn't Exist", args: args{ctx: context.TODO()}, fields: fields{logisticEvent: event}, wantErr: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a := &TransportStartedManager{
			...
			got, err := internal.Repo.GetFacilityExpectedParcelDocumentByID(context.TODO(), facilityExpectedParcel1.FacilityID)
			if err != nil {
				t.Errorf("TransportStartedManager.ManageEvent(). Cannot check test on doc id  %v", facilityID)
			}
			if !reflect.DeepEqual(got, expFacilityExpectedParcel) {
...
func TestTransportStartedManager_ManageEvent_KO_FacilityDoesntExist(t *testing.T) {
	facilityID := uuid.New().String()
	vehicleID := uuid.New().String()
	transportID := uuid.New().String()
...
func TestTransportStartedManager_ManageEvent_KO_VehicleTransportDoesntExist(t *testing.T) {
	facilityID := uuid.New().String()
	vehicleID := uuid.New().String()
	transportID := uuid.New().String()
