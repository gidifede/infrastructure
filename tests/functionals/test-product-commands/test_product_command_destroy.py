...
def arrange_destroy_cmd_fields(product_id):
...
def test_product_command_destroy_OK(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [ 
        DestroyProduct(
            product=product,
...
@pytest.mark.functional
def test_product_command_destroy_missing_product_KO(config,product_id):
    ## Arrange
    _, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
                timestamp=utils.get_timestamp(),
                location=location,
...
@pytest.mark.functional
def test_product_command_destroy_missing_location_KO(config,product_id):
    ## Arrange
    product, _ = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
                product=product,
                timestamp=utils.get_timestamp(),
...
@pytest.mark.functional
def test_product_command_destroy_missing_timestamp_KO(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
...
@pytest.mark.functional
def test_product_command_destroy_bad_timestamp_KO(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
...
@pytest.mark.functional
def test_product_command_destroy_missing_notes_KO(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
...
@pytest.mark.functional
def test_product_command_destroy_bad_notes_KO(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [
        DestroyProduct(
...
@pytest.mark.functional
def test_product_command_destroy_missing_api_key_OK(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [ 
        DestroyProduct(
...
@pytest.mark.functional
def test_product_command_destroy_wrong_api_key_OK(config,product_id):
    ## Arrange
    product, location = arrange_destroy_cmd_fields(product_id=product_id)
    cmds = [ 
        DestroyProduct(
