...
def arrange_make_ready_to_be_delivered_cmd_fields(product_id):
    return Product(product_id, 'Poste Delivery Business', 'BOX', [])


@pytest.mark.functional
@pytest.mark.skip(reason="bug LOI-504")
def test_product_command_make_ready_to_be_delivered_OK(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_missing_product_KO(config,product_id):
    ## Arrange
    _ = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
                timestamp=utils.get_timestamp(),
                attributes=[]
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_missing_timestamp_KO(config,product_id):
    ## Arrange
    product= arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_bad_timestamp_KO(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_missing_attributes_KO(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_bad_attributes_KO(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_missing_api_key_OK(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
...
@pytest.mark.functional
def test_product_command_make_ready_to_be_delivered_wrong_api_key_OK(config,product_id):
    ## Arrange
    product = arrange_make_ready_to_be_delivered_cmd_fields(product_id=product_id)
    cmds = [
        MakeProductReadyToBeDelivered(
